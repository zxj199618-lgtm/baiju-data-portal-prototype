...
assert(gatewaySource.includes("const UNUSABLE_MODEL_RE = /image|audio|realtime|vision|-distill-|codex-auto/") && gatewaySource.includes("function filterUsableModels(models)") && (gatewaySource.match(/filterUsableModels\(result\.models\)/g) || []).length >= 4, "所有「自动拉取模型」的路径（迁移/启动同步/后台补拉/拉取模型/拆分）都必须走统一的图像语音过滤，不能有漏网的");
